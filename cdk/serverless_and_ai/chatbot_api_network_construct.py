...
class ChatBotNetworkConstruct(Construct):
    ...
    def __init__(self, scope: Construct, stack_id: str) -> None:
        ...
        self._scope = scope
        subnets = [
            aws_ec2.SubnetConfiguration(name="public-subnet-1",
        ...
        self.vpc_sg = aws_ec2.SecurityGroup(self,
                                            "ChatBotVPCSecurityGroup",
                                            vpc=self.vpc,
                                            allow_all_outbound=True)
        self.vpc_sg.connections.allow_from(
            aws_ec2.Peer.ipv4(self._VPC_CIDR),
            aws_ec2.Port.tcp(self._REDIS_PORT),
            description='Allow Redis access from witihin VPC CIDR')
